...
public class CodeController {
     ...
     * Subject : 공통콤보 seq 조회
     */
    @GetMapping("/combo/seq")
    public ResponseEntity<List<Pair>> getComboSeq(@ModelAttribute CodeDto codeDto) {
        List<Pair> codes = codeService.getComboSeq(codeDto);
        return new ResponseEntity<>(codes, HttpStatus.OK);
    }
    ...
    public ResponseEntity<Page<CodeHeader>> getGroupCodeListPage(@RequestBody CodeDto codeDto) {

        Pageable pageable = PageRequest.of(codeDto.getPage(), codeDto.getSize());
        Page<CodeHeader> all = codeHeaderRepository.findAll(pageable);
        return new ResponseEntity<>(all, HttpStatus.OK);
    }

    @PostMapping("/list/page/test")
    public ResponseEntity<Page<CodeDto>> getGroupCodeListPageTest(@RequestBody CodeDto codeDto) {

        Pageable pageable = PageRequest.of(codeDto.getPage(), codeDto.getSize());
        Page<CodeDto> codeDtos = codeQdslRepository.searchGroupCodePageSimple(codeDto, pageable);
        return new ResponseEntity<>(codeDtos, HttpStatus.OK);
    }

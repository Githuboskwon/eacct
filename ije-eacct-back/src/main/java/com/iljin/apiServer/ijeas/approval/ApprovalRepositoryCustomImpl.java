...
public class ApprovalRepositoryCustomImpl implements ApprovalRepositoryCustom {
    ...
    public List<ApprovalHeaderDto> getApprTodoList(ApprovalHeaderDto approvalHeaderDto) {
        ...
        String docTypeCd = approvalHeaderDto.getSlipType();
        ...
        String draftUserId = approvalHeaderDto.getDraftId();
        ...
        sb.append("SELECT HD.DOC_TYPE_CD" + "       ,A.DETAIL_NM AS DOC_TYPE" + "       ,HD.DOC_STAT_CD"
                ...
                + "       AND DT.APPR_SEQ = " +
                "(SELECT MIN(T.APPR_SEQ) FROM TB_APPR_DT T WHERE T.APPR_NO = HD.APPR_NO AND T.APPR_FG_CD = 'X')")
        ;
    ...
    public List<ApprovalHeaderDto> getApprDoneList(ApprovalHeaderDto approvalHeaderDto) {
        ...
        String searchDtmTo = approvalHeaderDto.getSearchDtmTo();
        String docTitleNm = approvalHeaderDto.getDocTitleNm();
        String docStatCd = approvalHeaderDto.getSlipStatus();
        String draftUserId = approvalHeaderDto.getDraftId();
    ...
    public List<ApprovalHeaderDto> getApprHeader(String docNo) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT HD.APPR_GROUP_ID" +
                ", HD.SLIP_NO AS SLIP_NO" +
                ", HD.SLIP_TYPE AS SLIP_TYPE" +
                ", HD.DRAFT_ID" +
                ", HD.NEXT_APP_USER_ID" +
                ", HD.NEXT_STAGE" +
                ", (SELECT DETAIL_NM FROM tb_code_dt WHERE COMP_CD = :compCd AND GROUP_CD = 'SLIP_TYPE_CD' AND DETAIL_CD = HD.SLIP_TYPE) AS SLIP_TYPE_NM " +
                ", HD.SLIP_STATUS " +
                ", (SELECT DETAIL_NM FROM tb_code_dt WHERE COMP_CD = :compCd AND GROUP_CD = 'SLIP_STAT_CD' AND DETAIL_CD = HD.SLIP_STATUS) AS STATUS " +
                ", HD.DOC_TITLE_NM AS DOC_TITLE_NM " +
                ", HD.REF_USER_ID" +
            " FROM TB_APPR_HD HD " +
            " WHERE HD.SLIP_NO = :docNo " +
    ...
    public List<ApprovalDetailDto> getApprDetail(String docNo) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT DT.APPR_NO " +
                    ", DT.APPR_TYPE_CD" +
                    ", (SELECT DETAIL_NM FROM TB_CODE_DT WHERE COMP_CD = :compCd AND GROUP_CD = 'APPR_TYPE_CD' AND DETAIL_CD = DT.APPR_TYPE_CD) AS APPR_TYPE " +
                    ", DT.APRVER_ID " +
                    ", DT.APRVER_NM AS APRVER_USER " +
                    ", DT.A_APRVER_ID " +
                    ", DT.A_APRVER_NM AS A_APRVER_USER" +
                    ", DT.SLIP_STATUS " +
                    ", (SELECT DETAIL_NM FROM tb_code_dt WHERE COMP_CD = :compCd AND GROUP_CD = 'SLIP_STAT_CD' AND DETAIL_CD = DT.SLIP_STATUS) AS APPR_STATUS " +
                    ", TO_CHAR(DT.APPR_DTM, 'YYYY-MM-DD HH24:MI:SS') AS APPR_DTM" +
                    ", APPR_DESC " +
            " FROM TB_APPR_DT DT " +
            " WHERE DT.SLIP_NO = :docNo " );
    ...
    public List<ApprovalEmployeeDto> getApprovalEmpList() {
        ...
        sb.append("" + " WITH RECURSIVE cte (CCTR_CD, CCTR_NM, UPPER_CD, LVL_CD, DEPTH_NM) AS " + " ( "
                ...
                + "   FROM cte DEPT, TB_MST_EMP EMP" + "  WHERE DEPT.CCTR_CD = EMP.HIT_DEPT_CD"
    ...
    public List<ApprovalEmployeeDto> getApprovalDeptTreeList() {
        ...
        sb.append("WITH cte (CCTR_CD, CCTR_NM, UPPER_CD, LVL_CD, DEPTH_NM) AS " + "    ( "
                + "    SELECT CCTR_CD " + "           , CCTR_NM " + "           , UPPER_CD " + "           , LVL_CD "
                + "           , CCTR_NM AS DEPTH_NM" + "    FROM tb_mst_cctr"
                //+ "    WHERE UPPER_CD = '0000000000'"
                + " WHERE COMP_CD = :compCd AND LVL_CD = '3' AND CCTR_CD NOT IN ('100991','nversion_EL','O000000113','O000000032','O000000013','O000000078','O000000033')" //트리 시작 지점 설정
                + "    UNION ALL " + "    SELECT r.CCTR_CD " + "           , r.CCTR_NM " + "           , r.UPPER_CD "
                + "                                 , r.lvl_cd      , cte.DEPTH_NM || '-' || r.CCTR_NM AS DEPTH_NM"
                + "    FROM tb_mst_cctr r" + "    INNER JOIN cte ON" + " r.COMP_CD = :compCd AND r.UPPER_CD = cte.CCTR_CD AND r.CCTR_CD NOT IN ('nversion_EL','O000000113','O000000032','O000000013','O000000078','O000000033')" + "    ) "
    ...
    public List<ApprovalRuleDto> getApprRuleLines(String loginId, ApprovalRuleDto approvalRuleDto) {
        ...
        sb.append("" + "SELECT T.*" + "  FROM (" + "    SELECT AR.RULE_SEQ," + "           AR.MAX_AMT,"
                ...
                + "               WHEN AR.APRVER_CLASS_CD = 'Dept' THEN (SELECT EMP_NO FROM TB_MST_EMP WHERE HIT_DEPT_CD = AR.APRVER_CLASS_VAL AND JOB_CD LIKE '%장')"
                + "               WHEN AR.APRVER_CLASS_CD = 'Emp' THEN AR.APRVER_CLASS_VAL"
                + "           ELSE A.EMP_NO END APRVER_ID," + "           CASE"
                + "               WHEN AR.APRVER_CLASS_CD = 'Dept' THEN (SELECT EMP_NM FROM TB_MST_EMP WHERE HIT_DEPT_CD = AR.APRVER_CLASS_VAL AND JOB_CD LIKE '%장')"
                ...
                + "               WHEN AR.APRVER_CLASS_CD = 'Dept' THEN (SELECT JOB_NM FROM TB_MST_EMP WHERE HIT_DEPT_CD = AR.APRVER_CLASS_VAL AND JOB_CD LIKE '%장')"
                ...
                + "               WHEN AR.APRVER_CLASS_CD = 'Dept' THEN (SELECT DEPT_NM FROM TB_MST_EMP WHERE HIT_DEPT_CD = AR.APRVER_CLASS_VAL AND JOB_CD LIKE '%장')"
                ...
                + "             FROM TB_MST_CCTR CC, TB_MST_EMP EM" + "            WHERE CC.CCTR_CD = (SELECT HIT_DEPT_CD"
                + "                                  FROM TB_MST_EMP"
                + "                                 WHERE EMP_NO = :loginId)" + "              AND CC.UPPER_CD <> ''"
                + "              AND EM.HIT_DEPT_CD = CC.CCTR_CD" + "              AND EM.JOB_CD LIKE '%장'"
                ...
                + "                                  FROM TB_MST_CCTR A"
                + "                                 WHERE A.CCTR_CD = (SELECT HIT_DEPT_CD"
                + "                                                      FROM TB_MST_EMP"
                + "                                                     WHERE EMP_NO = :loginId))"
                + "              AND CC.UPPER_CD <> ''" + "              AND EM.HIT_DEPT_CD = CC.CCTR_CD"
    ...
    @Override
    public List<ApprovalHeaderDto> getApprCnt(BigDecimal apprGroupId, String nextStage) {
        StringBuilder sb = new StringBuilder();
        sb.append(" SELECT " +
            "   COUNT(*) AS CNT " +
            " FROM TB_APPR_HD " +
            " WHERE 1 = 1 " +
            " AND APPR_GROUP_ID = :apprGroupId " +
            " AND NEXT_STAGE = :nextStage " +
            " AND NEXT_APP_USER_ID IN ( SELECT LOGIN_ID FROM A_USER WHERE LOGIN_ID = :loginId AND COMP_CD = :compCd " +
            " UNION ALL SELECT DISTINCT GIVE_USER_ID FROM TB_MST_DELEGATE WHERE DELEGATE_STAT_CD = '2' " +
            " AND START_DATE <= :date " +
            " AND END_DATE >= :date " +
            " AND RECEIVE_USER_ID = :loginId )");

        String date = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        Query query = entityManager.createNativeQuery(sb.toString());
        query.setParameter("apprGroupId", apprGroupId);
        query.setParameter("nextStage", nextStage);
    ...
    @Override
    public void callConfirmCheck(BigDecimal apprGroupId) {
        StoredProcedureQuery storedProcedureQuery = entityManager.createNamedStoredProcedureQuery("APPS.CBO_SP_SLIP_PKG.CONFIRM_CHECK");
        storedProcedureQuery.setParameter("p_approval_group_id", apprGroupId);
        storedProcedureQuery.setParameter("p_final_flag", "Y");
        storedProcedureQuery.execute();

        String xErrFlag = (String) storedProcedureQuery.getOutputParameterValue("x_err_flag");
        String xErrStep = (String) storedProcedureQuery.getOutputParameterValue("x_err_step");
        String xErrMsg = (String) storedProcedureQuery.getOutputParameterValue("x_err_msg");

        if(!"S".equals(xErrFlag)) {
            throw new ApprovalException("Step : " + xErrStep + " Msg : " + xErrMsg);
        }
    }

    @Override
    public void callSlipCancelConcurrent(BigDecimal apprGroupId) {
        StoredProcedureQuery storedProcedureQuery = entityManager.createNamedStoredProcedureQuery("APPS.CBO_SP_SLIP_PKG.SLIP_CANCEL_CONCURRENT");
        storedProcedureQuery.setParameter("p_approval_group_id", apprGroupId);
        storedProcedureQuery.setParameter("p_cancel_user_name", util.getLoginId());
        storedProcedureQuery.setParameter("p_slip_process", "SP");
        storedProcedureQuery.execute();
        ...
        if(!"S".equals(xErrFlag)) {
            throw new ApprovalException(xErrMsg);
        }
    }
}

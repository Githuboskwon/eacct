...
public interface AuthorityRepository extends JpaRepository<Role, RoleKey> {
    ...
    @Query(value = "" +
            ...
            "   AND user.userName LIKE CONCAT('%' , :value1 , '%') " +
            "   AND emp.deptNm LIKE CONCAT('%' , :value2 , '%') " +
            " ORDER BY CASE WHEN emp.upperDeptCd IS NULL THEN '0' ELSE emp.upperDeptCd END ASC, emp.deptCd ASC, LENGTH(emp.jobNm) DESC")
    List<Object[]> getUserInfoByAuthority(@Param("compCd") String compCd, @Param("roleCd") String roleCd, @Param("value1") String value1, @Param("value2") String value2);
}

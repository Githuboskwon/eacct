  ...
  <div class="inner-box">
    ...
    <div class="search-form">
      <div class="form-group">
        <label class="control-label-req" id="standardMonth">만기일자</label>
          ...
          <div class="search_box_wrap">
            ...
            <div class="search_box_pop">
              <div class="search_title">
                <span class="search_tit">만기일자</span>
              </div>
      ...
      <div class="table-b">
        <div class="table-header">
          <div class="table-name">
            <h3 class="ico_table_name">전자채권내역</h3>
          </div>
          <div style="float:left; font-size: 18px; color: salmon; margin-left:15px;"> 선택 총합계 : {{$numeral(totalAmt).format('0,0')}} </div>
...
export default {
  ...
  methods: {
    ...
    compareDate(value){
      if(!value){
        this.$swal({ type: 'info', text: '만기일자를 입력해주세요.' });
        return false;
      }
    },
    goSearch(){
      ...
      this.$http.post('/api/erp/slip/bond/list', this.form)
          .then(response => {
    ...
    pull() {
      ...
      this.form.searchFrom = this.$moment(this.searchDt[0]).format("YYYY-MM-DD HH:mm:ss");
      this.form.searchTo = this.$moment(this.searchDt[1]).format("YYYY-MM-DD HH:mm:ss");
      this.$http.post('/api/erp/slip/23', this.form)
          .then((res) => {
      ...
      if(list.length > 0){
        this.$swal({
          ...
          if (r.value) {
            this.$store.commit('loading')
            vm.result = list;
            this.$http.post(`/api/erp/slip/delete/23`, vm.result)
                .then((response) => {
    ...
    onRowSelected(params) {
      ...
      console.log(params.data.totalAmt)
      console.log(params.data.sumAmt);

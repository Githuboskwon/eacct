              ...
              <div class="btn-wrap" >
                ...
                <table class="table">
                  ...
                  <tr>
                    ...
                    <td colspan="3" >{{data.bankNm}}</td>
                    <th style="width: 140px;">은행지점</th>
                    <td colspan="3">{{data.bankBranchNm}}</td>
                  </tr>
                  <tr>
                    <th>은행계좌</th>
                    <td colspan="3">{{data.bankAcctNum}}</td>
                    <th>발행일자</th>
                    <td colspan="3">{{getDateFormat(data.postingDt)}}</td>
                  </tr>
                  <tr>
                    <th>통화</th>
                    <td colspan="3">{{data.usedCur}}</td>
                    <th>발행금액</th>
                ...
                <div class="table-b">
                  <div class="grid-wrap">
                    <ag-grid-vue ref="grid" style="width: 100%; height:470px;" class="ag-theme-alpine grid_search_height_350"
                                rowSelection="single" 
  ...
  methods: {
    ...
    getAppr() {
      return new Promise((resolve) => {
        this.$http.post('/api/erp/slip/bulk/detail', {
    ...
    approval() {
      ...
      const docTitleNm = this.isEmpty(that.remark) === "" ? that.totalAmt : this.isEmpty(that.remark) + ' / ' + that.totalAmt;
        ...
        this.$swal({
          ...
          if (result.value) {
            ...
            this.$http.put(`/api/appr/detail/req`, {
              ...
              docTitleNm : this.isEmpty(that.remark) === "" ? that.totalAmt : this.isEmpty(that.remark) + ' / ' + that.totalAmt,
    ...
    getTotalAmt(){
      if(this.data.usedCur === "KRW"){
        this.totalAmt = this.getNumberFormat(this.data.usedAmt)
      }else{
        this.totalAmt = this.getDoubleNumberFormat(this.data.usedForAmt)
      }
    },
    isEmpty(value){

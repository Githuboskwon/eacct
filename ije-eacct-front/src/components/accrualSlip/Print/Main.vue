...
<!--&lt;!&ndash;                <el-button type="success" v-if="isReuse" @click="reuse" icon="el-icon-user">전표 재사용</el-button>&ndash;&gt;-->
...
<!--              <el-button type="success" size="midium" icon="el-icon-printer" @click="print">출력</el-button>-->
<!--            </div>-->
          <div class="table-area" ref="printContent">
            <!-- 결재선 -->
            ...
            <!-- 증빙파일 등 -->
            <!-- 파일 업로드 -->
            <div class="file-upload mt20" v-if="showWingsAtchPop">
...
export default {
    ...
    methods: {
        ...
        print() {
          this.$nextTick(() => {
            ...
            setTimeout(() => {
              ...
              this.convertInputsToText();
              const printContent = this.$refs.printContent;
              if (!printContent) {
                alert('프린트할 내용을 찾을 수 없습니다.');
                return;
              }
        ...
        convertInputsToText() {
          const inputs = this.$refs.printContent.querySelectorAll('input, textarea');
          ...
          // 특정 태그와 클래스를 제거
          const italicTags = this.$refs.printContent.querySelectorAll('i , label, .ag-header-icon');
          italicTags.forEach(iTag => {
            iTag.remove();
          });

          const detailHeader = this.$refs.printContent.querySelectorAll('.ag-header-cell-label');
          detailHeader.forEach(header => {
            header.style.display = "inline";
            header.style.alignContent = "center";
          });

          const files = this.$refs.printContent.querySelectorAll('.file-name');
          files.forEach(files => {
            files.style.padding ='11px 25px 11px 25px';
            files.style.width = "100%";
          });

          const fileNames = this.$refs.printContent.querySelectorAll('.file-cta .file-label');
          fileNames.forEach(fileNames => {
            fileNames.style.padding ='0 10px 0 0';
          });
        ...
        printImage(imageData) {
          ...
          <html>
            <head>
              ...
              <style>
                @media print {
                  ...
                  img {
                    width: 100%; /* 좌우 여백을 위해 이미지 너비 조정 */
                    height: auto;
                    /*margin: 5mm; !* 이미지 주위에 여백 추가 *!*/
            ...
            <body>
              <img src="${imageData}" style="width:calc(100% - 20mm); height:auto; margin:10mm;">
            </body>
          </html>
          `);

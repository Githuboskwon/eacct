                    ...
                    .then(data => {
                        ...
                        rowData.forEach((x, i) => {
                            ...
                            if(subRowNode.data.awtAccountName.indexOf('고용보험료') > -1) {
                              subRowNode.setDataValue(`preTaxAmt`, rowData.filter(f => f.withholdingTaxCode && f.withholdingTaxCode === '2').map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                                ...
                                subRowNode.setDataValue(`preTaxAmt`, rowData.filter(f => f.withholdingTaxCode && !['2', '3', '4'].includes(f.withholdingTaxCode)).map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                              // subRowNode.setDataValue(`preTaxAmt`, rowData.filter(f => f.withholdingTaxCode && f.withholdingTaxCode != '2').map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                            }
                              ...
                              subRowNode.setDataValue(`preTaxAmt`, rowData.filter(f => f.withholdingTaxCode && f.withholdingTaxCode === '3').map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                              // subRowNode.setDataValue(`preTaxAmt`, rowData.map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                            }
                            else if(subRowNode.data.awtAccountName.indexOf('의료보험료') > -1) {
                              subRowNode.setDataValue(`preTaxAmt`, rowData.filter(f => f.withholdingTaxCode && f.withholdingTaxCode === '4').map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                              // subRowNode.setDataValue(`preTaxAmt`, rowData.map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                            }
                        });
                    ...
                    if(this.value.rowData && this.value.rowData.length > 0) {
                        this.value.etcawt.forEach((x, i) => {
                            ...
                            if(subRowNode.data.awtAccountName.indexOf('고용보험료') > -1) {
                                ...
                                subRowNode.setDataValue(`preTaxAmt`, this.value.rowData.filter(f => f.withholdingTaxCode && !['2', '3', '4'].includes(f.withholdingTaxCode)).map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                            }
                            else if(subRowNode.data.awtAccountName.indexOf('주민세') > -1) {
                                subRowNode.setDataValue(`preTaxAmt`, this.value.rowData.filter(f => f.withholdingTaxCode && !['2', '3', '4'].includes(f.withholdingTaxCode)).map(x => this.$numeral(x.supplyAmount).value()).reduce((prev, next) => {return prev + next}, 0));
                            }
    ...
    methods: {
         ...
         */
         onCellValueChanged(params) {
            ...
            const newData = this.value.etcawt[rowIndex];

            if(field === 'preTaxAmt') {
                const preTaxAmt = this.$numeral(newData.preTaxAmt).value();
                const taxRate = this.$numeral(newData.taxRate).value();

                const afterTotTaxAmt = parseInt(parseInt(preTaxAmt * (taxRate / 100)) / 10) * 10;

                if(newData.awtAccountName.indexOf('소득세') > -1) {
                    this.setGridDataValue(rowIndex, 'afterTaxAmt', afterTotTaxAmt < 1000 ? 0 : afterTotTaxAmt);
                } else if(newData.awtAccountName.indexOf('주민세') > -1) {
                    this.setGridDataValue(rowIndex, 'afterTaxAmt', afterTotTaxAmt < 100 ? 0 : afterTotTaxAmt);
                } else if(newData.awtAccountName.indexOf('국민연금') > -1) {
                    this.setGridDataValue(rowIndex, 'afterTaxAmt', afterTotTaxAmt);
                } else if(newData.awtAccountName.indexOf('의료보험료') > -1) {
                    this.setGridDataValue(rowIndex, 'afterTaxAmt', afterTotTaxAmt);
                } else {
                    this.setGridDataValue(rowIndex, 'afterTaxAmt', afterTotTaxAmt);
                }
        ...
        handleRowDataChanged(event) {
            const index = this.value.etcawt.length - 1;
        ...
        makeBottomTotal() {
            ...
            let sum = [{
                rn: '합계'
                , taxRate: this.value.etcawt.map(x => x.taxRate).reduce((prev, next) => {return prev + next}, initValue)
                , preTaxAmt: this.value.etcawt.filter(f => f.awtAccountName.indexOf('고용보험료') > -1).map(x => x.preTaxAmt).reduce((prev, next) => {return prev + next}, initValue)
              , afterTaxAmt: this.value.etcawt.map(x => x.afterTaxAmt).reduce((prev, next) => {return prev + next}, initValue)
            }];
         ...
         */
        handleScroll(event) {
            const grid = document.getElementById('etcawt-agGrid');
            if (grid) {

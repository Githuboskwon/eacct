<template>
    <div>
        <div class="table-area" style="margin-top: 0px;">
            <appr-bundle-header :doc-type="docType" :apprHeader="apprHeader" :appr-line="apprLine" v-model="data"/>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            queryApprLineRule() {
                ...
                this.$http.post('/api/appr/line/list', {docTypeCd:docTypeCd})
                    .then(response => {
                      if (Array.isArray(response.data) && response.data.length > 0) {
                        this.apprLine = response.data.filter(x => x.aprverId)

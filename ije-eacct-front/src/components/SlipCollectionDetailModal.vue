...
export default {
  ...
  methods: {
    ...
    getAppr() {
      this.$http.post('/api/erp/slip/collection/detail', {
    ...
    makeSumData(){
      ...
      let sum = [{
        rn: 'TOTAL',
        functionalAmtDr: this.data.dtList.reduce((prev, next) => {return Number(prev) + Number(next.functionalAmtDr)}, initValue),
        functionalAmtCr: this.data.dtList.reduce((prev, next) => {return Number(prev) + Number(next.functionalAmtCr)}, initValue)
      }]
    ...
    approval() {
      ...
      let docTitleNm = that.data.remark + ' / ' + this.$numeral(that.data.functionalAmtCr).format('0,0');

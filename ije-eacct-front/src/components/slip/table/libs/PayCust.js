...
const td = {
  ...
  <td :colspan="colspan" v-else>
    <div class="td-s-thumb search-area">
      <input class="input input-bg mr5 Rt-M rs-mt2" type="text" v-model="value.evdCustCd" style="width:74px;" disabled="disabled" />
      <div class="ip-box ip-box-w02 rs-mt2" style="width:calc(100% - 108px)">
...
const td_pay_chk = {
  ...
  <th style="background-color: #ffffff;">
    <input type="checkbox" :id="id" v-model="value.usePayCust" :disabled="readonly" @click="checkInput" />
  ...
  computed: {
    ...
    custAccount: {
      get(){
        return this.value.custBnkCd + "|" + this.value.custAcctNo;
      },
      set(newValue){
        return newValue
      }
  ...
  methods: {
    checkInput(event) {
      ...
      if ($(event.target).is(":checked")) {
        ...
        this.$parent.evdCusChangeEvent();
      }else {
        // eslint-disable-next-line no-undef
        $(".actual").attr("disabled", true);
        this.applyInitialize(true);
        this.$parent.evdCusChangeEvent();
      }
      /*
    ...
    applyInitialize(force){
      if (force === true) this.value.payCustNm = '';
      if (this.value.payCustNm === '') {
        this.value.payCustCd = '';
        this.value.payCustBizNo = '';
      }
    },
  },
  created() {
    //this.value.usePayCust = !!(this.value.payCustCd !== undefined)
    this.value.usePayCust = !!(this.value.payCustCd !== '' &&this.value.payCustCd !== null && this.value.payCustCd !== undefined)
  }
}
...
const td_pay = {
  ...
  template: `
  <td class="bd-l-none" v-if="readonly && !!(value.payCustCd) ">{{ value.payCustCd }} / {{ value.payCustNm }} / {{ value.payCustBizNo }}</td>
  <td class="bd-l-none" v-else-if="readonly && !!(!value.payCustCd) "></td>
  <td class="bd-l-none" v-else>
    <div class="td-s-thumb search-area">
      <input class="input input-bg mr5 Rt-M rs-mt2" type="text" v-model="value.payCustCd" style="width:74px;" disabled="disabled" />
      <div class="ip-box ip-box-w02 rs-mt2">
        <input type="text" class="input actual" v-model="value.payCustNm" @keypress.enter="query" :disabled="!value.usePayCust" />
        <button class="icon is-right actual" @click="modal" :disabled="!value.usePayCust"><i class="fas fa-search"></i></button>
      </div>
      <button class="remove_text trash-input rs-mb2 actual" @click="apply()" :disabled="!value.usePayCust">
        <i class="far fa-trash-alt"></i>
      </button>
    </div>
  </td>
...
export default {
  ...
  render(createElement, context) {
    ...
    return context.props.usePayCust ? [the, tde, tde_pay_1, tde_pay_2] : [the, tde, tde_pay_1, tde_pay_2]
  }
}

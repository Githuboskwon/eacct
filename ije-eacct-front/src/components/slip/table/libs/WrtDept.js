...
const td = {
  ...
  template: `
  <td :colspan="colspan" v-if="readonly">{{ value.wrtDeptCd }} / {{ value.wrtDeptNm }}</td>
  <td :colspan="colspan" v-else>  
    <div class="td-s-thumb search-area" v-if="value.slipTypeCd === 'E2' || value.slipTypeCd === 'E5'" >
      <input class="input input-bg mr5 Rt-M rs-mt2" type="text" v-model="value.wrtDeptCd" style="width:66px;" disabled="disabled" />
      <div class="ip-box ip-box-w02 rs-mt2" style="width:calc(100% - 71px)">
        <input type="text" class="input input-bg mr5" v-model="value.wrtDeptNm" disabled="disabled"/>
      </div>
    </div>
    <div class="td-s-thumb search-area" v-else>
      <input class="input input-bg mr5 Rt-M rs-mt2" type="text" v-model="value.wrtDeptCd" style="width:66px;" disabled="disabled" />
      <div class="ip-box ip-box-w02 rs-mt2" style="width:calc(100% - 100px)">
        <input type="text" class="input" v-model="value.wrtDeptNm" @keypress.enter="query(value.wrtDeptNm)" />
        <button class="icon is-right" @click="modal(value.wrtDeptNm)"><i class="fas fa-search"></i></button>
      </div>
  ...
  methods: {
    apply(data) {
      data = data || {}
      this.value.wrtDeptCd = data.cctrCd
      this.value.wrtDeptNm = data.cctrNm
  ...
  created(){
    if(!this.value.wrtDeptCd) {
      this.value.wrtDeptCd = this.$store.state.loginInfo.loginDeptCd;
      this.value.wrtDeptNm = this.$store.state.loginInfo.loginDeptNm;
    }
  }
}
